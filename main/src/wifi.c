...
void wifi_connect() {
	...
	//first attempt: set clear CONNECTING_FAILED bit and CONNECTING bit
	else {
...
/*
	...
	Sets the DISCONNECT_CALLED bit so the event handler doesn't attempt to reconnect
...
esp_err_t wifi_startAP(char *ssid, char *pass) {
	...
	esp_err_t err = esp_wifi_start();
	if (err == ESP_OK) {
		ESP_LOGI(TAG, "WiFi started in AP mode. SSID: '%s', pass: '%s'", ssid, pass);
